$(function () {
  ...
  $('.quiz__link.quiz__link--next').on('click', function (e) {
    e.preventDefault();
    $('.quiz__test--active .quiz__input-checkbox').each(function (item) {
      if ($(item).is(':checked')) {
        $('.quiz__link--next').removeClass('disabled');
      }
    });
  ...
  //последний блок тестов added class change button for get promo code
  $('.quiz__items').children(':last').addClass('change-button');
