$(function () {
  ...
  $('.quiz__item--contact').on('change', function () {
    var empty = $('.quiz__item--contact')
      .find('.quiz__item--contact')
      .filter(function () {
        return this.value === '';
      });

    if (empty.length > 0) {
      $('.quiz__link--next').prop('disabled', true);
    } else {
      $('.quiz__link--next').prop('disabled', false);
    }
  });
});
